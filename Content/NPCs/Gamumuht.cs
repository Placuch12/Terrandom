    ...
    public class Gamumuht : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (target != null && !target.dead && NPC.HasValidTarget && NPC.Distance(target.Center) < 500f)
            {
                if (axeThrowTimer >= 90)
                {
                    ...
                    {
                        int projectile = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, direction * 10, ModContent.ProjectileType<Projectiles.Hostile.GamumuhtAxe>(), 10, 1f, Main.myPlayer);
                    }
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			bestiaryEntry.Info.AddRange([
				...
				new FlavorTextBestiaryInfoElement("This jungle civilization hasn't figured out that there are other weapons than axes yet.")
			]);
		}

        public override void ModifyNPCLoot(NPCLoot npcLoot) {
            npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<JungleSap>(), 1, 1, 2));
